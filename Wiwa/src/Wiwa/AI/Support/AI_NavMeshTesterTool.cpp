#include <wipch.h>
#include "AI_NavMeshTesterTool.h"
#include "../RecastObjects/AI_RecastCommon.h"
#include "Recast.h"
...
void NavMeshTesterTool::init(RecastCommon* sample)
{
	m_sample = sample;
...
{
	if (ImGui::CollapsingHeader("Pathfinding Queries", ImGuiTreeNodeFlags_DefaultOpen))
	{

		if (ImGui::RadioButton("Pathfind Follow", reinterpret_cast<int*>(&m_toolMode), TOOLMODE_PATHFIND_FOLLOW))
		{
			m_toolMode = TOOLMODE_PATHFIND_FOLLOW;
			recalc();
		}
		if (ImGui::RadioButton("Pathfind Straight", reinterpret_cast<int*>(&m_toolMode), TOOLMODE_PATHFIND_STRAIGHT))
		{
		...
		if (ImGui::RadioButton("Pathfind Sliced", reinterpret_cast<int*>(&m_toolMode), TOOLMODE_PATHFIND_SLICED))
		{
		...
		if (ImGui::RadioButton("Distance to Wall", reinterpret_cast<int*>(&m_toolMode), TOOLMODE_DISTANCE_TO_WALL))
		{
		...
		if (ImGui::RadioButton("Raycast", reinterpret_cast<int*>(&m_toolMode), TOOLMODE_RAYCAST))
		{
		...
		if (ImGui::RadioButton("Find Polys in Circle", reinterpret_cast<int*>(&m_toolMode), TOOLMODE_FIND_POLYS_IN_CIRCLE))
		{
			m_toolMode = TOOLMODE_FIND_POLYS_IN_CIRCLE;
			recalc();
		}
	
		if (ImGui::RadioButton("Find Polys in Shape", reinterpret_cast<int*>(&m_toolMode), TOOLMODE_FIND_POLYS_IN_SHAPE))
		{
		...
		if (ImGui::RadioButton("Find Local Neighbourhood", reinterpret_cast<int*>(&m_toolMode), TOOLMODE_FIND_LOCAL_NEIGHBOURHOOD))
		{
		...
		ImGui::SameLine();
		ImGui::Text("Start Pos -> x: %f  y: %f  z: %f", m_spos[0], m_spos[1], m_spos[2]);
		...
		ImGui::SameLine();
		ImGui::Text("End Pos -> x: %f  y: %f  z: %f", m_epos[0], m_epos[1], m_epos[2]);
		ImGui::Separator();
	}
}
...
void NavMeshTesterTool::handleRender()
{
	...
	if (m_toolMode == TOOLMODE_PATHFIND_FOLLOW)
	{
		duDebugDrawNavMeshPoly(&dd, *m_navMesh, m_startRef, endCol);
		duDebugDrawNavMeshPoly(&dd, *m_navMesh, m_endRef, startCol);
